...
当创建新数据集时，在被称为shuffle的过程中，宽转换会导致数据在分区之间移动。数据通过网络发送到其他节点并写到磁盘，从而导致网络和磁盘I/O，使shuffle成为一项成本高昂的操作。典型的宽转换包括`groupBy`、`agg`、`sortBy`和`orderBy`等。下面是一个按航空公司分组计算航班数量的宽转换。
```scala
